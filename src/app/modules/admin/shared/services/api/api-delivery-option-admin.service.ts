...
export class ApiDeliveryOptionAdminService {
  ...
  post(newDeliveryOption: CreateDeliveryOption): Observable<ResponseEntity> {
    return this.http.post<ResponseEntity>(UTAIDA_PROJECT.back.url + "/delivery-option", newDeliveryOption)
  }

  getAll(): Observable<DeliveryOptionDto[]> {
    return this.http.get<DeliveryOptionDto[]>(UTAIDA_PROJECT.back.url + "/delivery-option")
  }

  changeAvailability(deliveryOption : DeliveryOptionDto): Observable<ResponseEntity>{
    return this.http.put<ResponseEntity>(UTAIDA_PROJECT.back.url + "/delivery-option/change-availability", deliveryOption)
  }

  changeExpress(deliveryOption : DeliveryOptionDto): Observable<ResponseEntity>{
    return this.http.put<ResponseEntity>(UTAIDA_PROJECT.back.url + "/delivery-option/change-express", deliveryOption)
  }

  put(deliveryOptionUpdated: DeliveryOptionDto): Observable<ResponseEntity> {
    return this.http.put<ResponseEntity>(UTAIDA_PROJECT.back.url + "/delivery-option", deliveryOptionUpdated)
  }

  delete(deliveryOptionToDelete: DeliveryOptionDto): Observable<ResponseEntity>{
    return this.http.delete<ResponseEntity>(UTAIDA_PROJECT.back.url + "/delivery-option/" + deliveryOptionToDelete.slug)
  }
}
