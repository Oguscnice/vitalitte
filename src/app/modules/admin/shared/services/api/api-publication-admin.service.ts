...
export class ApiPublicationAdminService {
  ...
  post(publication : CreatePublication): Observable<ResponseEntity>{
    return this.http.post<ResponseEntity>(UTAIDA_PROJECT.back.url + "/publications", publication)
  }

  put(publication : PublicationDto): Observable<ResponseEntity>{
    return this.http.put<ResponseEntity>(UTAIDA_PROJECT.back.url + "/publications/" + publication.slug, publication)
  }

  changeSpotlighted(publication : PublicationDto): Observable<ResponseEntity>{
    return this.http.put<ResponseEntity>(UTAIDA_PROJECT.back.url + "/publications/spotlighted", publication)
  }

  delete(publicationSlug : PublicationDto['slug']): Observable<ResponseEntity>{
    return this.http.delete<ResponseEntity>(UTAIDA_PROJECT.back.url + "/publications/" + publicationSlug)
  }
}
