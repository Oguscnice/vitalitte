...
export class ApiReviewAdminService {
  ...
  getAllReviewStatus(): Observable<ReviewDto['status'][]> {
    return this.http.get<ReviewDto['status'][]>(UTAIDA_PROJECT.back.url + "/review-status")
  }

  changeStatus(review: ReviewDto): Observable<ResponseEntity> {
    return this.http.post<ResponseEntity>(UTAIDA_PROJECT.back.url + "/reviews/change-status", review)
  }
}
